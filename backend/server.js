...
// Enhanced JWT middleware with automatic token refresh
    ...
    if (err.name === 'TokenExpiredError') {
      try {
        ...
        if (decoded && decoded.username) {
          ...
          const userExists = users.find(u => u[0] === decoded.username);
          ...
          if (userExists) {
            ...
            // Add new token to response headers for frontend to use
            res.setHeader('X-New-Token', newToken);
            
            console.log('Token auto-refreshed for user:', decoded.username);
            return next();
          }
        }
      } catch (refreshError) {
...
// Login with longer token expiry
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    res.json({ 
      username, 
      sessionToken,
      expiresIn: 24 * 60 * 60 * 1000 // Send expiry info to frontend
    });
  } catch (error) {
...
app.post('/api/refresh-token', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // If verification fails, try to decode (for expired tokens)
      if (err.name === 'TokenExpiredError') {
...
// Verify token endpoint for frontend to check token status
