...
const HomePage = ({
  ...
  const calculateDuePayment = (rowData, months) => {
    ...
    return Math.round(duePayment * 100) / 100;
  };
  ...
  // Fetch fresh data for filtered clients when filters change
  useEffect(() => {
    const fetchFilteredClientData = async () => {
      if (!sessionToken || !currentYear || (!monthFilter && !statusFilter && !searchQuery)) return;

      setIsLoadingPayments(true);
      console.log(`HomePage.jsx: Fetching fresh data for filtered clients:`, {
        monthFilter,
        statusFilter,
        searchQuery,
        year: currentYear,
      ...
      try {
        const clientNames = filteredData.map(row => row.Client_Name);
        if (clientNames.length === 0) {
          console.log('No clients to fetch after filtering.');
          setPaymentsData([]);
          return;
        }

        const response = await axios.post(
          `${BASE_URL}/get-payments-by-clients`,
          { clientNames, year: currentYear },
          {
        ...
        console.log(`Fetched fresh data for ${freshData.length} clients:`, freshData);

        const normalizedData = freshData.map((row) => {
          const normalizedRow = { ...row };
          months.forEach((month) => {
            const capitalizedMonth = month.charAt(0).toUpperCase() + month.slice(1);
            if (normalizedRow[month] === undefined && normalizedRow[capitalizedMonth] !== undefined) {
              normalizedRow[month] = normalizedRow[capitalizedMonth];
              delete normalizedRow[capitalizedMonth];
            }
            normalizedRow[month] = normalizedRow[month] || "";
          });
          return normalizedRow;
        });

        setPaymentsData(normalizedData);
        setCachedData(paymentsCacheKey, normalizedData);
        ...
        console.log(`Updated paymentsData with ${normalizedData.length} rows.`);
      } catch (error) {
        console.error('HomePage.jsx: Error fetching filtered client data:', error);
        setLocalErrorMessage(
          error.response?.data?.error || 'Failed to load filtered client data.'
        );
      } finally {
    ...
    fetchFilteredClientData();
  }, [filteredData, sessionToken, currentYear, monthFilter, statusFilter, searchQuery, getCacheKey, setCachedData, setPaymentsData, setLocalErrorMessage, months]);
  ...
  useEffect(() => {
    const loadPaymentsData = async () => {
      ...
      try {
        ...
        const normalizedData = paymentsData.map((row) => {
          const normalizedRow = { ...row };
          months.forEach((month) => {
            const capitalizedMonth = month.charAt(0).toUpperCase() + month.slice(1);
            if (normalizedRow[month] === undefined && normalizedRow[capitalizedMonth] !== undefined) {
        ...
        console.log(`HomePage.jsx: Payments fetched for year ${currentYear}: ${normalizedData.length} items`);
        if (normalizedData.length > 0) {
          console.log("🔍 Sample Normalized Row:", normalizedData[0]);
        }
      } catch (error) {
    ...
    loadPaymentsData();
  }, [sessionToken, currentYear, fetchPayments, getCacheKey, getCachedData, setPaymentsData, refreshTrigger, lastRefreshTrigger, months]);
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((row, idx) => {
                    const originalIndex = paymentsData.findIndex(
                      (r) => r.Client_Name === row.Client_Name
                    );
                    console.log(`Rendering row: idx=${idx}, originalIndex=${originalIndex}, Client_Name=${row.Client_Name}`);
                    return (
                      <tr
                        key={`${row?.Client_Name || "unknown"}-${originalIndex}`}
                        ...
                        {months.map((month, colIndex) => (
                          ...
                          >
                            <input
                              type="text"
                              value={
                                localInputValues[`${originalIndex}-${month}`] !== undefined
                                  ? localInputValues[`${originalIndex}-${month}`]
                                  : row?.[month] || ""
                              }
                              onChange={(e) =>
                                handleInputChange(originalIndex, month, e.target.value)
                              }
                              ...
                              placeholder="0.00"
                              title={
                                pendingUpdates[`${originalIndex}-${month}`]
                                  ? "Saving..."
                                  : ""
                              }
                            />
