...
const BATCH_DELAY = 700; // Reduced for faster batch processing
const BATCH_SIZE = 5;
...
const HomePage = ({
  ...
  const [toastMessage, setToastMessage] = useState(null); // New state for toast notifications
  ...
  // Updated calculateDuePayment to use Amount_To_Be_Paid * 12 - Σ(monthly payments)
  const calculateDuePayment = (rowData, months) => {
    ...
    // Expected payment = Amount_To_Be_Paid * 12
    ...
    async (abortSignal) => {
      ...
      return createDedupedRequest(requestKey, async () => {
        ...
        try {
          ...
          setToastMessage({ message: userMessage, type: "error" });
  ...
  const handleAddNewYear = useCallback(async () => {
    ...
    try {
      ...
      if (paymentsData.length === 0 && expectedClientCount > 0) {
        ...
        setToastMessage({ message: errorMsg, type: "warning" });
        alert(errorMsg);
      } else {
        setToastMessage({ message: `Year ${newYear} added successfully with ${paymentsData.length} clients.`, type: "success" });
  ...
  }, [currentYear, sessionToken, getCacheKey, searchUserYears, setPaymentsData, setCurrentYear]);
  ...
  const ToastMessage = ({ message, type, onDismiss }) => {
    ...
    return (
      <div className={`fixed top-4 right-4 p-4 rounded-lg border ${bgColor} z-50 max-w-sm`}>
  ...
  const debugDuePayment = (rowIndex, stage, value, rowData) => {
    console.log(`Due Payment Debug - Row ${rowIndex} at ${stage}: ${value}`, {
      rowData,
      totalPayments: months.reduce((sum, m) => sum + (parseFloat(rowData[m]) || 0), 0),
  ...
  const processBatchUpdates = useCallback(
    async () => {
      ...
      try {
          ...
          setPaymentsData((prev) => {
            ...
            successfulUpdates.forEach(({ rowIndex, updatedRow }) => {
              if (updatedRow && updated[rowIndex]) {
                console.log(`HomePage.jsx: Updating row ${rowIndex} with Due_Payment: ${updatedRow.Due_Payment}`);
                ...
                const mappedRow = {
                  ...
                  January: updatedRow.january !== undefined ? updatedRow.january : updated[rowIndex].January || "",
                  February: updatedRow.february !== undefined ? updatedRow.february : updated[rowIndex].February || "",
                  March: updatedRow.march !== undefined ? updatedRow.march : updated[rowIndex].March || "",
                  April: updatedRow.april !== undefined ? updatedRow.april : updated[rowIndex].April || "",
                  May: updatedRow.may !== undefined ? updatedRow.may : updated[rowIndex].May || "",
                  June: updatedRow.june !== undefined ? updatedRow.june : updated[rowIndex].June || "",
                  July: updatedRow.july !== undefined ? updatedRow.july : updated[rowIndex].July || "",
                  August: updatedRow.august !== undefined ? updatedRow.august : updated[rowIndex].August || "",
                  September: updatedRow.september !== undefined ? updatedRow.september : updated[rowIndex].September || "",
                  October: updatedRow.october !== undefined ? updatedRow.october : updated[rowIndex].October || "",
                  November: updatedRow.november !== undefined ? updatedRow.november : updated[rowIndex].November || "",
                  December: updatedRow.december !== undefined ? updatedRow.december : updated[rowIndex].December || "",
                  Due_Payment: parseFloat(updatedRow.Due_Payment || 0).toFixed(2)
                };
                
                debugDuePayment(rowIndex, "Backend Update", mappedRow.Due_Payment, mappedRow);
          ...
          setLocalInputValues((prev) => {
            ...
            successfulUpdates.forEach(({ updates, rowIndex, updatedRow }) => {
              updates.forEach(({ month }) => {
                ...
                newValues[key] = updatedRow[month] !== undefined ? updatedRow[month] : "";
              });
            });
            return newValues;
          });
        ...
        setToastMessage({ message: `Batch update failed: ${error.message}`, type: "error" });
      } finally {
        setIsUpdating(false);
      }
    },
  ...
  // New function to export payment data as CSV
  const exportToCsv = useCallback(() => {
    ...
    link.setAttribute("download", `payments_${currentYear}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    
    setToastMessage({ message: "Payment data exported successfully!", type: "success" });
  }, [filteredData, currentYear, months]);
    ...
    async (clientName, clientEmail, clientPhone, type, year, notifyStatuses, duePayment) => {
      ...
      if (hasValidPhone) {
        ...
        try {
          ...
          if (!verifyResponse.data.isValidWhatsApp) {
            ...
            setToastMessage({
              message: `Cannot send WhatsApp message to ${clientName}: Phone number is not registered with WhatsApp.`,
              type: "error"
            });
            isValidWhatsApp = false;
          }
        } catch (verifyError) {
          console.error(`HomePage.jsx: WhatsApp verification failed for ${clientPhone} (${clientName})`, verifyError);
          setToastMessage({
            message: `Failed to verify WhatsApp status for ${clientName}: ${verifyError.response?.data?.error || verifyError.message}`,
            type: "error"
          });
          isValidWhatsApp = false;
        }
        ...
        if (isValidWhatsApp) {
          try {
            ...
            setToastMessage({
              message: `WhatsApp notification sent successfully to ${clientName}`,
              type: "success"
            });
          } catch (whatsappError) {
            console.error(`HomePage.jsx: WhatsApp attempt failed for ${clientPhone} (${clientName})`, whatsappError);
            setToastMessage({
              message: `Failed to send WhatsApp message to ${clientName}: ${whatsappError.response?.data?.error || whatsappError.message}`,
              type: "error"
            });
          }
        }
      ...
      if (!notificationSent && hasValidEmailAddress) {
        try {
          ...
          notificationSent = true;
          setToastMessage({
            message: `Email notification sent successfully to ${clientName}`,
            type: "success"
          });
        } catch (emailError) {
          console.error(`HomePage.jsx: Email failed for ${clientEmail} (${clientName})`, emailError);
          setToastMessage({
            message: `Failed to send email notification to ${clientName}: ${emailError.response?.data?.error || emailError.message}`,
            type: "error"
          });
        }
        ...
        setToastMessage({
          message: `No notification sent for ${clientName}: No valid phone or email provided.`,
          type: "warning"
        });
      } else if (!notificationSent) {
        console.log(`HomePage.jsx: Email not attempted for ${clientName} due to invalid email`);
        setToastMessage({
          message: `No notification sent for ${clientName}: Email address invalid or missing.`,
          type: "warning"
        });
      }
  ...
  const debouncedUpdate = useCallback(
    (rowIndex, month, value, year) => {
      if (!paymentsData.length) {
        ...
        setToastMessage({ message: "Please wait for data to load before making updates.", type: "error" });
        return;
      }
      
      if (!paymentsData[rowIndex]) {
        console.warn("HomePage.jsx: Invalid rowIndex:", rowIndex);
        setToastMessage({ message: "Invalid row index.", type: "error" });
        return;
      }
  ...
  const handleInputChange = useCallback(
    (rowIndex, month, value) => {
      ...
      if (trimmedValue !== "" && (isNaN(parsedValue) || parsedValue < 0)) {
        setToastMessage({ message: "Please enter a valid non-negative number.", type: "error" });
        return;
      }
      ...
      setPaymentsData((prev) => {
        ...
        debugDuePayment(rowIndex, "Optimistic Update", newDuePayment, rowData);
        rowData.Due_Payment = newDuePayment.toFixed(2);
        
        updatedPayments[rowIndex] = rowData;
        console.log(`HomePage.jsx: Optimistic update for row ${rowIndex}, ${month}:`, {
          monthValue: trimmedValue,
    ...
    [debouncedUpdate, currentYear, setPaymentsData, months]
  );
  ...
  useEffect(() => {
    const loadPaymentsData = async () => {
      ...
      try {
        ...
        setToastMessage({
          message: error.response?.data?.error || 'Failed to load payments data.',
          type: "error"
        });
  ...
  const handleAddType = async () => {
    ...
    if (!newType.trim()) {
      setToastMessage({ message: "Type name cannot be empty.", type: "error" });
      return;
    }
    if (newType.trim().length > 50) {
      setToastMessage({ message: "Type name too long.", type: "error" });
      return;
    }
    ...
    try {
      ...
      setNewType("");
      setSearchQuery("");
      setToastMessage({ message: `Type ${capitalizedType} added successfully.`, type: "success" });
      ...
      {isTypeModalOpen && (
        ...
        >
          ...
          >
            ...
            {toastMessage && (
              <p className="text-red-500 mb-4 text-sm">{toastMessage.message}</p>
            )}
            <input
              type="text"
              value={newType}
